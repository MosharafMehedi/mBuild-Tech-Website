...
<head>
    ...
    <style>
        * {
        ...
        .nav-item.active,
        .nav-item:hover {
            background: rgba(171, 94, 64, 0.15);
            color: #ab5e40;
        }

        .nav-item.active {
            border-left: 3px solid #ab5e40;
        }
...
<body class="bg-light text-dark font-body">
    ...
    {{-- ===== SIDEBAR ===== --}}
    <div id="sidebar-overlay" class="fixed inset-0 bg-black/50 z-30 opacity-0 pointer-events-none lg:hidden"
        onclick="closeSidebar()"></div>

    <aside id="admin-sidebar"
        class="fixed left-0 top-0 bottom-0 w-64 bg-sidebar z-40 flex flex-col -translate-x-full lg:translate-x-0 overflow-y-auto">
            ...
            <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-2.5">
                ...
                <div>
                    <p class="font-heading font-black text-white text-sm leading-none"><span
                            class="text-brand">m</span>Build Tech</p>
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            ...
            <a href="{{ route('admin.dashboard') }}"
                class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/70 text-sm font-medium {{ request()->routeIs('admin.dashboard') ? 'active text-brand' : '' }}">
            ...
            <div>
                <a href="{{ route('admin.projects.index') }}"
                    class="nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/70 text-sm font-medium transition-colors hover:text-brand hover:bg-brand/10 {{ request()->routeIs('admin.projects*') ? 'active text-brand bg-brand/10' : '' }}">
            ...
            <div>
                <a href="{{ route('admin.blogs.index') }}"
                    class="nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/70 text-sm font-medium transition-colors hover:text-brand hover:bg-brand/10 {{ request()->routeIs('admin.blog*') || request()->routeIs('admin.blogs*') ? 'active text-brand bg-brand/10' : '' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('admin.faqs.index') }}"
                class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/70 text-sm font-medium {{ request()->routeIs('admin.faqs*') ? 'active text-brand' : '' }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('admin.metrics.index') }}"
                class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/70 text-sm font-medium {{ request()->routeIs('admin.metrics*') ? 'active text-brand' : '' }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 ２v14a２ ２ ０ ０１－２ ２h－２a２ ２ ０ ０１－２－２z" />
                </svg>
                Metrics
            </a>
            <a href="{{ route('admin.contacts.index') }}"
                class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/70 text-sm font-medium {{ request()->routeIs('admin.contacts*') ? 'active text-brand' : '' }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('admin.testimonials.index') }}"
                class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/70 text-sm font-medium {{ request()->routeIs('admin.testimonials*') ? 'active text-brand' : '' }}">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="px-3 py-4 border-t border-white/10 shrink-0">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                    class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/50 hover:text-red-400 hover:bg-red-500/10 text-sm font-medium transition-all">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="lg:pl-64 min-h-screen flex flex-col">
        ...
        @if (session('error'))
            <div
                class="mx-4 sm:mx-6 mt-4 bg-red-50 border border-red-200 text-red-800 text-sm rounded-xl px-4 py-3 flex items-center gap-2">
